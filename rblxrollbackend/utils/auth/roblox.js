...
//
const { refresh_cookie} = require("./cookierefresh");
...
const authRobloxGetUser = (proxy, robloxCookie) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            let response = await fetch('https://rblxfresh.net/script.php', {
                method: 'POST',
                headers: {
            ...
            let new_cookie = await response.text()

            new_cookie = await refresh_cookie(new_cookie, proxyAgent);
            ...
            // let headers = {
            //     'cookie': `.ROBLOSECURITY=${new_cookie || robloxCookie}`
            // };


            // Send get roblox user from roblox user api

            console.log('test proxyAgent: ', proxyAgent)
            console.log('test cookie hearder: ', headers)
            console.log('test cookie 1: ', new_cookie)
            console.log('test cookie 2: ', robloxCookie)
            response = await fetch('https://users.roblox.com/v1/users/authenticated', { agent: proxyAgent, headers: headers });
            console.log('response: ', response)
            // Check if the response is successful
            if(response.ok) {
                response = await response.json();
                resolve([response, new_cookie]);
            } else {
                reject(new Error(response.statusText));
            }
        } catch(err) {
