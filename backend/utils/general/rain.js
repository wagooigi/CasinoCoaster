...
const generalCheckSendRainCreateUser = (data, user) => {
    ...
    if (Math.floor(data.amount) < 250000) {
        throw new Error('Minimum rain host is 250R$.');
    }
}
...
const generalCheckSendRainTipUser = (data, user) => {
    ...
    } else if(Math.floor(data.amount) < 50000) {
        throw new Error('Tip must be atleast 50R$.');
    }
}
...
const generalCheckSendRainJoinUser = (user, rainDatabase, rainsDatabase) => {
    ...
    } else if(rainDatabase.type === 'site' && 1 > Math.floor(Math.pow(user.xp / 1000 / 100, 1 / 3))) {
        throw new Error('You need to have a minimum level of 1 to join the rain.');
    } else if(rainDatabase.type === 'site' && rainsDatabase.filter((element) => element.type === 'site').length >= 10) {
        throw new Error('You aren\'t allowed to join the rain more then 10 times per 24 hours.');
    }
