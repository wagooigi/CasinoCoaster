    ...
    socket.on('sendRobuxDeposit', async(data, callback) => {
        ...
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                ...
                try {
                    const user = await User.findById(socket.decoded._id).select('roblox username avatar rank proxy mute ban stats').lean();
                    socketCheckUserData(user, true);
    ...
    socket.on('sendRobuxWithdraw', async(data, callback) => {
        ...
        if(socket.decoded !== undefined && socket.decoded !== null) {
            try {
                ...
                await socketCheckAntiSpam(socket.decoded._id);
                try {
                    const user = await User.findById(socket.decoded._id).select('roblox username avatar rank balance limits proxy mute ban stats').lean();
                    socketCheckUserData(user, true);
