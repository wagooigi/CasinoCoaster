...
module.exports = (io) => {
    ...
    router.post('/coinpayments', rateLimiterMiddleware, async(req, res) => {
        try {
            ...
            //axios.post("https://buxdrop.com/coinpayment/verify-crypto", req.body, {headers: req.headers}).then()
            ...
            try {
                ...
                if(req.body.ipn_type === 'deposit' && dataDatabase[0] !== null && dataDatabase[1] === null && req.body.status >= 100) {
                    ...
                    const amount = Math.floor((amountFiat / 1.9) * 1000 * 1.15); // 15%bonus
                    ...
                    if(amount >= 1000000){
                        Rain.deleteMany({ 'participants.user': dataDatabase[0].user._id, state: 'completed', updatedAt: { $gte: new Date(new Date().getTime() - 1000 * 60 * 60 * 24) }})
                    }
                }
            ...
            try {
                ...
                if(transactionDatabase !== null && transactionDatabase.state === 'created') {
                    ...
                    if(req.body.status === 'success') {
                        ...
                        // Get transaction amount in robux
                        const amount = Math.floor((amountFiat / 1.9) * 1000);
